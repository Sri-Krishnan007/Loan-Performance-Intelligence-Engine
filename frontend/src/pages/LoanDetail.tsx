...
import {
  getLoanById,
  getExceptions,
  getAIRecommendationForException,
  getAuditTrail,
  performReviewAction
...
export const LoanDetail: React.FC = () => {
  ...
  const { data: aiRecommendation } = useQuery({
    queryKey: ['aiRec', activeException?.id],
    queryFn: () => getAIRecommendationForException(activeException?.id || ''),
    enabled: !!activeException
  });
  ...
  const handleApplyAI = () => {
    if (!aiRecommendation || !activeException) return;

    if (aiRecommendation.suggestedAction === 'correct') {
      const field = activeException.affectedField;
      const value = aiRecommendation.suggestedValue;

      reviewMutation.mutate({
        loanId: loan.loanId,
        exceptionId: activeException.id,
        action: 'edit_record',
        notes: `AI Recommendation Accepted: Corrected "${field}" to "${value}".`,
        reviewerName: 'Lead Reviewer',
        updatedFields: { [field]: value }
      });
    } else if (aiRecommendation.suggestedAction === 'waive') {
      reviewMutation.mutate({
        loanId: loan.loanId,
        exceptionId: activeException.id,
        action: 'waive_exception',
        notes: 'AI Recommendation Accepted: Waived exception after matching criteria verification.',
        reviewerName: 'Lead Reviewer'
      });
    }
  };
  ...
  const saveInlineEdit = () => {
    ...
    reviewMutation.mutate({
      ...
      exceptionId: activeException.id,
      action: 'edit_record',
      notes: `Reviewer manually modified field "${editingField}" to "${editValue}".`,
      reviewerName: 'Lead Reviewer',
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-6">
          ...
          <div className="bg-slate-900 border border-slate-700 rounded-2xl p-5 shadow-lg relative overflow-hidden flex flex-col justify-between min-h-[420px]">
            ...
            <div>
              ...
              {!activeException ? (
                ...
                <div className="py-12 text-center text-xs text-slate-500">
                  Calculating recommendation matrices...
                </div>
              ) : (
                <div className="space-y-4 text-xs leading-relaxed">
                  ...
                  <div className="border-t border-slate-800 pt-3">
                    ...
                    <div className="mt-1.5 grid grid-cols-2 gap-2 p-2 bg-slate-950 rounded border border-slate-800 font-mono text-[10px]">
                      ...
                      <div>
                        <span className="text-slate-555 block text-right">Tape Value</span>
                  ...
                  <div className="border-t border-slate-800 pt-3">
                    ...
                    <p className="text-slate-300 mt-1.5 font-medium">{aiRecommendation.explanation}</p>
                  </div>
                  ...
                  <div className="border-t border-slate-800 pt-3">
                    ...
                    <div className="mt-1.5 p-2 bg-slate-950 border border-slate-800 rounded font-mono text-[10px] text-slate-400 relative">
                      <span>{`Reconciled ${activeException.affectedField} to ${aiRecommendation.suggestedValue ?? 'waived'} based on Copilot analysis.`}</span>
                      <button
                        type="button"
                        onClick={() => setNotes(`Reconciled ${activeException.affectedField} to ${aiRecommendation.suggestedValue ?? 'waived'} based on Copilot analysis.`)}
                        className="absolute right-1.5 top-1 px-1 py-0.5 bg-slate-900 border border-slate-700 rounded text-[9px] font-bold text-indigo-400 hover:text-indigo-300 transition"
                      >
                        Apply
                      </button>
                    </div>
                    ...
                    <div className="p-2.5 bg-slate-950 rounded-lg border border-slate-800 flex items-center justify-between">
                      <div>
                        <span className="font-bold text-indigo-400 uppercase tracking-wider text-[9px] block">
                          Suggested Action: {aiRecommendation.suggestedAction}
                        </span>
                        {aiRecommendation.suggestedValue !== undefined && (
                          <span className="text-slate-200 font-bold block mt-0.5">
                            Set value → <strong className="text-emerald-400 font-extrabold">{aiRecommendation.suggestedValue}</strong>
                          </span>
                        )}
                      </div>
                      <div className="text-right">
                        <span className="text-[9px] text-slate-550 block font-semibold uppercase">Confidence</span>
                        <span className="text-lg font-extrabold text-slate-200 block">{aiRecommendation.confidence}%</span>
                      </div>
                    </div>
                  </div>
                </div>
            ...
            {activeException && aiRecommendation && (
