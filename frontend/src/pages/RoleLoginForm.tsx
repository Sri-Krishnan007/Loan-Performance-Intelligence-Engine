...
export const RoleLoginForm: React.FC<RoleLoginFormProps> = ({ role, title, description, redirect }) => {
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      const isValidOperator = role === 'operator' && cleanedEmail === 'operator@demo.local' && password === 'Operator@123';
      const isValidReviewer = role === 'reviewer' && cleanedEmail === 'reviewer@demo.local' && password === 'Reviewer@123';
      const isValidConsumer = role === 'consumer' && cleanedEmail === 'consumer@demo.local' && password === 'Consumer@123';

      if (isValidOperator) {
        localStorage.setItem('user_role', 'operator');
        localStorage.setItem('user_email', cleanedEmail);
        localStorage.setItem('user_name', 'Data Operator Account');
        showToast('Logged in as Data Operator', 'success');
        navigate(redirect);
      } else if (isValidReviewer) {
        localStorage.setItem('user_role', 'reviewer');
        localStorage.setItem('user_email', cleanedEmail);
        localStorage.setItem('user_name', 'Lead Reviewer Account');
        showToast('Logged in as Exception Reviewer', 'success');
        navigate(redirect);
      } else if (isValidConsumer) {
        localStorage.setItem('user_role', 'consumer');
        localStorage.setItem('user_email', cleanedEmail);
        localStorage.setItem('user_name', 'Data Auditor');
        showToast('Logged in as Data Consumer', 'success');
        navigate(redirect);
      } else {
